...
type announce struct {
	...
	fetch61     blockRequesterFn  // [ele/61] Fetcher function to retrieve an announced block
...
func (f *Fetcher) Notify(peer string, hash common.Hash, number uint64, time time.Time,
	blockFetcher blockRequesterFn, // ele/61 specific whole block fetcher
...
func (f *Fetcher) FilterBlocks(blocks types.Blocks) types.Blocks {
	glog.V(logger.Detail).Infof("[ele/61] filtering %d blocks", len(blocks))
			...
			for peer, hashes := range request {
				if glog.V(logger.Detail) && len(hashes) > 0 {
					...
					if f.fetching[hashes[0]].fetch61 != nil {
						glog.V(logger.Detail).Infof("[ele/61] Peer %s: fetching blocks %s", peer, list)
					} else {
				...
				go func() {
					...
					if fetchBlocks != nil {
						// Use old ele/61 protocol to retrieve whole blocks
						blockFetchMeter.Mark(int64(len(hashes)))
						fetchBlocks(hashes)
					} else {
						// Use new ele/62 protocol to retrieve headers first
						for _, hash := range hashes {
