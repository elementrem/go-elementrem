...
func newWorker(config *core.ChainConfig, coinbase common.Address, ele core.Backend) *worker {
	worker := &worker{
		config:         config,
		ele:            ele,
		mux:            ele.EventMux(),
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	accounts := self.ele.AccountManager().Accounts()
...
func (self *worker) commitNewWork() {
	...
	*/
	...
	for _, tx := range self.ele.TxPool().GetTransactions() {
	...
	*/
	...
	self.ele.TxPool().RemoveTransactions(work.lowGasTxs)
	self.ele.TxPool().RemoveTransactions(work.failedTxs)
