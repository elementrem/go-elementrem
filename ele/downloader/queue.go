...
type queue struct {
	...
	hashPool    map[common.Hash]int // [ele/61] Pending hashes, mapping to their insertion index (priority)
	hashQueue   *prque.Prque        // [ele/61] Priority queue of the block hashes to fetch
	hashCounter int                 // [ele/61] Counter indexing the added hashes to ensure retrieval order
	...
	headerDonePool  map[uint64]struct{}            // [ele/62] Set of the completed header fetches
	...
	stateProcessors int32            // [ele/63] Number of currently running state processors
	stateSchedLock  sync.RWMutex     // [ele/63] Lock serialising access to the state scheduler
