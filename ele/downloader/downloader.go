...
var (
	...
	hashTTL        = 3 * time.Second     // [ele/61] Time it takes for a hash request to time out
	blockTargetRTT = 3 * time.Second / 2 // [ele/61] Target time for completing a block retrieval request
	blockTTL       = 3 * blockTargetRTT  // [ele/61] Maximum time allowance before a block request is considered expired
	...
	maxQueuedHashes   = 32 * 1024 // [ele/61] Maximum number of hashes to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	newPeerCh     chan *peer
	hashCh        chan dataPack        // [ele/61] Channel receiving inbound hashes
	blockCh       chan dataPack        // [ele/61] Channel receiving inbound blocks
	...
	blockWakeCh   chan bool            // [ele/61] Channel to signal the block fetcher of new tasks
...
func New(stateDb eledb.Database, mux *event.TypeMux, hasHeader headerCheckFn, hasBlockAndState blockAndStateCheckFn,
...
func (d *Downloader) RegisterPeer(id string, version int, head common.Hash,
	getRelHashes relativeHashFetcherFn, getAbsHashes absoluteHashFetcherFn, getBlocks blockFetcherFn, // ele/61 callbacks, remove when upgrading
...
func (d *Downloader) syncWithPeer(p *peer, hash common.Hash, td *big.Int) (err error) {
	...
	default:
		...
		glog.V(logger.Error).Infof("Unsupported ele protocol: %d", p.version)
		return errBadPeer
	}
}
...
func (d *Downloader) fetchHeight61(p *peer) (uint64, error) {
	...
	for {
		...
		case <-d.stateCh:
		case <-d.receiptCh:
			// Ignore ele/{62,63} packets because this is ele/61.
...
func (d *Downloader) fetchHeight(p *peer) (*types.Header, error) {
	...
	for {
		...
		case <-d.hashCh:
		case <-d.blockCh:
			// Ignore ele/61 packets because this is ele/62+.
			// These can arrive as a late delivery from a previous sync.
		}
	}
}
