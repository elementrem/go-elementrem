...
func doInstall(cmdline []string) {
	...
	if runtime.Version() < "go1.4" && !strings.HasPrefix(runtime.Version(), "devel") {
		...
		log.Println("go-elementrem requires at least Go version 1.4 and cannot")
...
func doArchive(cmdline []string) {
	...
	if err := build.WriteArchive("gele-"+base, ext, geleArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive("gele-alltools-"+base, ext, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
}
...
func doTravisDebianSource(cmdline []string) {
	...
	switch {
	case os.Getenv("TRAVIS_REPO_SLUG") != "elementrem/go-elementrem":
	...
	doDebianSource([]string{
		"-signer", "Felix Lange (Gele CI Testing Key) <fjl@twurst.com>",
		"-buildnum", os.Getenv("TRAVIS_BUILD_NUMBER"),
		"-upload", "ppa:lp-fjl/gele-ci-testing",
		"-unstable", unstable,
	})
}
...
func doDebianSource(cmdline []string) {
	...
	tmpdir, err := ioutil.TempDir("", "ele-deb-build-")
	if err != nil {
		log.Fatal(err)
	}
