...
import (
	...
	"github.com/elementrem/go-elementrem/whisper"
...
func MakeElementbase(accman *accounts.Manager, ctx *cli.Context) common.Address {
	...
	account, err := MakeAddress(accman, elementbase)
	if err != nil {
...
func MakeSystemNode(name, version string, relconf release.Config, extra []byte, ctx *cli.Context) *node.Node {
	...
	// Configure the Elementrem service
	...
	eleConf := &ele.Config{
		...
		Elementbase:               MakeElementbase(accman, ctx),
	...
	case ctx.GlobalBool(DevModeFlag.Name):
		...
		if !ctx.GlobalIsSet(DataDirFlag.Name) {
			stackConf.DataDir = filepath.Join(os.TempDir(), "/elementrem_dev_mode")
		}
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		return ele.New(ctx, eleConf)
	}); err != nil {
		Fatalf("Failed to register the Elementrem service: %v", err)
	}
	if shhEnable {
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		Fatalf("Failed to register the Gele release oracle service: %v", err)
	}
	return stack
}
...
func MakeChainDatabase(ctx *cli.Context) eledb.Database {
	var (
		datadir = MustMakeDataDir(ctx)
	...
	chainDb, err := eledb.NewLDBDatabase(filepath.Join(datadir, "chaindata"), cache, handles)
	if err != nil {
...
func MakeChain(ctx *cli.Context) (chain *core.BlockChain, chainDb eledb.Database) {
	var err error
