...
// Hash and block fetchers belonging to ele/61 and below
...
type peer struct {
	...
	getRelHashes relativeHashFetcherFn // [ele/61] Method to retrieve a batch of hashes from an origin hash
	getAbsHashes absoluteHashFetcherFn // [ele/61] Method to retrieve a batch of hashes from an absolute position
	getBlocks    blockFetcherFn        // [ele/61] Method to retrieve a batch of blocks
...
func (p *peer) Fetch61(request *fetchRequest) error {
	...
	if p.version != 61 {
		panic(fmt.Sprintf("block fetch [ele/61] requested on ele/%d", p.version))
	}
