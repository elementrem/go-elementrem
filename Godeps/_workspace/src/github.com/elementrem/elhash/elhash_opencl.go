...
/*
  ...
  1. The initCL(...)  function configures one or more OpenCL device
     (for now only GPU) and loads the Elhash DAG onto device memory

  2. The Search(...) function loads a Elhash nonce into device(s) memory and
     executes the Elhash OpenCL kernel.
...
type OpenCLDevice struct {
	...
	dagBuf        *cl.MemObject // Elhash full DAG in device mem
...
type OpenCLMiner struct {
	...
	elhash *Elhash // Elhash full DAG & cache in host mem
...
func PrintDevices() {
	...
	if len(found) == 0 {
		...
		fmt.Printf("Found %v devices. Benchmark first GPU:       gele gpubench 0\n", len(found))
		fmt.Printf("Mine using all GPUs:                        gele --minegpu %v\n", idsFormat)
	}
}
...
func InitCL(blockNum uint64, c *OpenCLMiner) error {
	platforms, err := cl.GetPlatforms()
	if err != nil {
		return fmt.Errorf("Plaform error: %v\nCheck your OpenCL installation and then run gele gpuinfo", err)
	}
	...
	for _, p := range platforms {
		...
		if err != nil {
			return fmt.Errorf("Devices error: %v\nCheck your GPU drivers and then run gele gpuinfo", err)
		}
		for _, d := range ds {
	...
	c.elhash = pow
	dagSize := uint64(C.elhash_get_datasize(C.uint64_t(blockNum)))
	c.dagSize = dagSize

	for _, id := range c.deviceIds {
		if id > len(devices)-1 {
			return fmt.Errorf("Device id not found. See available device ids with: gele gpuinfo")
		} else {
...
func initCLDevice(deviceId int, device *cl.Device, c *OpenCLMiner) error {
	...
	kvs["DAG_SIZE"] = strconv.FormatUint(c.dagSize/elhashMixBytesLen, 10)
	kvs["ACCESSES"] = strconv.FormatUint(elhashAccesses, 10)
	...
	*/
	...
	searchKernelName = "elhash_search"
	hashKernelName = "elhash_hash"
	...
	dagPtr := unsafe.Pointer(c.elhash.Full.current.ptr.data)
...
func (c *OpenCLMiner) Search(block pow.Block, stop <-chan struct{}, index int) (uint64, []byte) {
	c.mu.Lock()
	newDagSize := uint64(C.elhash_get_datasize(C.uint64_t(block.NumberU64())))
		...
		if len(pending) == searchBufSize {
			...
			for i := uint32(0); i < nfound; i++ {
				...
				if checkNonce != 0 {
					...
					// executing the Elhash verification function (on the CPU).
					cache := c.elhash.Light.getCache(block.NumberU64())
...
func (c *OpenCLMiner) Verify(block pow.Block) bool {
	return c.elhash.Light.Verify(block)
}
