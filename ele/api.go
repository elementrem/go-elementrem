...
func stateAndBlockByNumber(m *miner.Miner, bc *core.BlockChain, blockNr rpc.BlockNumber, chainDb eledb.Database) (*state.StateDB, *types.Block, error) {
...
func (s *PublicElementremAPI) GasPrice() *big.Int {
	return s.gpo.SuggestPrice()
}

// GetCompilers returns the collection of available smart contract compilers
func (s *PublicElementremAPI) GetCompilers() ([]string, error) {
	solc, err := s.e.Solc()
	if err == nil && solc != nil {
...
func (s *PublicElementremAPI) CompileSolidity(source string) (map[string]*compiler.Contract, error) {
	solc, err := s.e.Solc()
	if err != nil {
		return nil, err
	}
...
func (s *PublicElementremAPI) ProtocolVersion() *rpc.HexNumber {
	return rpc.NewHexNumber(s.e.EleVersion())
}

// Hashrate returns the POW hashrate
func (s *PublicElementremAPI) Hashrate() *rpc.HexNumber {
...
func NewPublicTxPoolAPI(e *Elementrem) *PublicTxPoolAPI {
...
func NewPrivateAccountAPI(e *Elementrem) *PrivateAccountAPI {
...
func NewPublicBlockChainAPI(config *core.ChainConfig, bc *core.BlockChain, m *miner.Miner, chainDb eledb.Database, gpo *GasPriceOracle, eventMux *event.TypeMux, am *accounts.Manager) *PublicBlockChainAPI {
...
func NewPublicTransactionPoolAPI(e *Elementrem) *PublicTransactionPoolAPI {
...
func getTransaction(chainDb eledb.Database, txPool *core.TxPool, txHash common.Hash) (*types.Transaction, bool, error) {
...
// PrivateAdminAPI is the collection of Elementrem APIs exposed over the private
// admin endpoint.
...
func (api *PrivateAdminAPI) ExportChain(file string) (bool, error) {
	...
	if err := api.ele.BlockChain().Export(out); err != nil {
		return false, err
	}
	return true, nil
}
...
// of the Elementrem service.
func NewPrivateDebugAPI(config *core.ChainConfig, ele *Elementrem) *PrivateDebugAPI {
	return &PrivateDebugAPI{config: config, ele: ele}
}

// ChaindbProperty returns leveldb properties of the chain database.
func (api *PrivateDebugAPI) ChaindbProperty(property string) (string, error) {
	ldb, ok := api.ele.chainDb.(interface {
		LDB() *leveldb.DB
	})
	if !ok {
...
func (api *PrivateDebugAPI) TraceBlockByHash(hash common.Hash, config *vm.Config) BlockTraceResult {
	// Fetch the block that we aim to reprocess
	block := api.ele.BlockChain().GetBlock(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) traceBlock(block *types.Block, config *vm.Config) (bool, []vm.StructLog, error) {
	...
	statedb, err := state.New(blockchain.GetBlock(block.ParentHash()).Root(), api.ele.ChainDb())
	if err != nil {
		return false, collector.traces, err
	}
...
func (api *PrivateDebugAPI) SetHead(number uint64) {
	api.ele.BlockChain().SetHead(number)
}
...
func (api *PrivateDebugAPI) TraceTransaction(txHash common.Hash, logger *vm.LogConfig) (*ExecutionResult, error) {
	...
	block := api.ele.BlockChain().GetBlock(blockHash)
	if block == nil {
		return nil, fmt.Errorf("block %x not found", blockHash)
	}
	// Create the state database to mutate and eventually trace
	parent := api.ele.BlockChain().GetBlock(block.ParentHash())
	if parent == nil {
		return nil, fmt.Errorf("block parent %x not found", block.ParentHash())
	}
	stateDb, err := state.New(parent.Root(), api.ele.ChainDb())
	if err != nil {
		return nil, err
	}
	...
	for idx, tx := range block.Transactions() {
		...
		vmenv := core.NewEnv(stateDb, api.config, api.ele.BlockChain(), msg, block.Header(), vm.Config{Debug: true, Logger: *logger})
