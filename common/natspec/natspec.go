...
import (
	...
	"github.com/elementrem/go-elementrem/common/registrar"
	"github.com/elementrem/go-elementrem/crypto"
	"github.com/elementrem/go-elementrem/xele"
...
func GetNotice(xele *xele.XEle, tx string, http *httpclient.HTTPClient) (notice string) {
	ns, err := New(xele, tx, http)
	if err != nil {
		if ns == nil {
...
func New(xele *xele.XEle, jsontx string, http *httpclient.HTTPClient) (self *NatSpec, err error) {
	...
	content, err := FetchDocsForContract(contractAddress, xele, http)
	if err != nil {
		return
	}
...
func FetchDocsForContract(contractAddress string, xele *xele.XEle, client *httpclient.HTTPClient) (content []byte, err error) {
	// retrieve contract hash from state
	codehex := xele.CodeAt(contractAddress)
	codeb := xele.CodeAtBytes(contractAddress)
...
func (self *NatSpec) makeAbi2method(abiKey [8]byte) (mele *method) {
...
func (self *NatSpec) Notice() (notice string, err error) {
	...
	mele := self.makeAbi2method(abiKey)

	if mele == nil {
		err = fmt.Errorf("abi key does not match any method")
		return
	}
	notice, err = self.noticeForMethod(self.tx, mele.name, mele.Notice)
	return
}
